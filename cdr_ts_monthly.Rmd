...
cellareas <- sic*(25^2)                  #finding ice area in each cell in km^2

icearea <- apply(cellareas, 3, sum, na.rm = TRUE)    #summing ice area per month

icearea <- icearea[c(-1, -2)]            #Deleting nov and dec of 1978
time <- time[c(-1, -2)]                  #Deleting nov and dec of 1978

yearly <- matrix(icearea, nrow = 42, byrow = TRUE)

#icearea_min <- apply(yearly, 1, min)    #taking the minimum month

icearea_min <- yearly[ , 9]              #taking september

icearea_min <- icearea_min/1e+06         #making units million km^2

plot(1979:2020, icearea_min, type = 'b',
     main = 'September Sea Ice Area',
     xlab = 'Year',
